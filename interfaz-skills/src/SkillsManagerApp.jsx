...
const API_BASE_URL = "http://localhost:3000/api"; // Cambiar a la URL de producción si es necesario
...
function App() {
    ...
    const editarHabilidad = (id, nombreActual) => {
        ...
        if (nuevoNombre && nuevoNombre.trim()) {
            axios
                .put(`${API_BASE_URL}/skills/${id}`, { skill: nuevoNombre })
                .then(() => {
    ...
    const agregarSubhabilidad = () => {
        ...
        axios
            ...
            .then((response) => {
                setHabilidades(habilidades.map((habilidad) => {
                    if (habilidad.id === parseInt(habilidadSeleccionada)) {
                        return {
                            ...habilidad,
                            microskills: [...habilidad.microskills, { id: response.data.id, name: nuevaSubhabilidad }]
                        };
                    }
                    return habilidad;
                }));
            ...
            .catch((error) => {
                console.error("Error al agregar la subhabilidad:", error);
            });
    };
    ...
    const editarSubhabilidad = (idHabilidad, idSubhabilidad, nombreActual) => {
        ...
        if (nuevoNombre && nuevoNombre.trim()) {
            axios
                .put(`${API_BASE_URL}/skills/${idHabilidad}/microskills/${idSubhabilidad}`, {
                    name: nuevoNombre
                })
                .then(() => {
                    setHabilidades(habilidades.map((habilidad) => {
        ...
        <div className="container">
            ...
            {habilidadSeleccionada && (
                <>
                    <ul>
                        {habilidades
                            .filter((habilidad) => habilidad.id === parseInt(habilidadSeleccionada))
                            .map((habilidad) => (
                                <li key={habilidad.id} style={{ position: "relative" }}>
                                    <strong>{habilidad.name}</strong>
                                    <button
                                        className="action-btn edit"
                                        onClick={() => editarHabilidad(habilidad.id, habilidad.name)}
                                    >
                                        ✏️
                                    </button>
                                    <button
